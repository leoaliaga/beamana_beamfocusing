...
lib: libbeamfocusing.so 

libbeamfocusing.so: $(OBJS_LIB)
...
$(PROGS): % : src/%.o $(OBJS_LIB)  libbeamfocusing.so
	if [ ! -d bin ]; then mkdir -p bin; fi

	$(CC) -Wall -o bin/$@ $< $(DEPLIBS) -L$(WORKDIR)/lib -lbeamfocusing -L${DK2NU_LIB} -ldk2nuTree
